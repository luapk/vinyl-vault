...
- **Auth lock**: `supabase.js` overrides the supabase-js cross-tab auth lock with a no-op. **Do not swap in a `navigator.locks` lock without a re-entrancy guard.** supabase-js acquires the lock and then calls `getSession()` again from inside it; a re-entrant `exclusive` request waits on a lock its own caller holds and never resolves. That is why the default lock deadlocks, and a "bounded wait" variant (tried and reverted) is no better -- it just stalls every auth call, and every DB query that reads the session (profile, role, community), for the timeout duration. A correct replacement needs a module-level depth counter that short-circuits when this context already holds the lock, tested in a real browser against both a tab and the installed PWA. Known trade-off of the no-op: a tab and the PWA can refresh the rotated refresh token concurrently and trip Supabase reuse detection ("session expired"). That is an inconvenience, not a data risk -- unsynced records survive a dead session and re-upload automatically.
